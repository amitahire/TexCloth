...
//#define RAND() ((rand()%2)/10.0f) //Intresting - try %2 and %10 etc.... intresteing effects.
...
ParticleEmitter::ParticleEmitter(void)	{
	...
	*/
	...
	//texture = SOIL_load_OGL_texture("../../Textures/nyan.png",
	//	SOIL_LOAD_AUTO,SOIL_CREATE_NEW_ID,SOIL_FLAG_COMPRESS_TO_DXT);
}
...
void ParticleEmitter::Update(float msec)	{
	nextParticleTime -= msec + 30;	//some time has passed! -- INTRESTING effect with "+ 1000" -- Swish Hit by an particle.
	...
	*/
	...
	for(std::vector<Particle *>::iterator i = particles.begin(); i != particles.end();/*No I++ here!!! */) {
		...
		if(p->colour.w <= 0.0f) {
			...
			p->position += p->direction*(msec*particleSpeed/10 + 100); //ORI - 100 (last element) -- Put the value to 10 - Intresting effect of slow mo.
...
Particle* ParticleEmitter::GetFreeParticle()	{
	...
	//INTRESTINg - COOL effect.
	p->colour		= Vector4(1, 0.7 , 0.7, 1.0); // snow white
	p->direction	= initialDirection;
	p->direction.x += ((RAND()-RAND()) * particleVariance);
	p->direction.y += ((RAND()-RAND()) * particleVariance + 10); // Add 10 for ray gun.
	...
	p->position.ToZero();
	//p->position = Vector3(2.0, 2.0, 2.0);
