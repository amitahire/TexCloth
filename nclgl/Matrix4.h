...
public:
	...
	//The following Inverse onle work for for matrix with diagonal elements.
	Matrix4 Inverse();
	...
	Matrix4 GetTransposedRotation() {
		...
		/*( 3 lines) Flips the values 'around' the diagonal of the matrix,
		into a new matrix which is then returned. */
	...
	Vector3 GetForwardRotationVector();
	Vector3 GetUpRotationVector();
	Vector3 GetXVector(); 
};
