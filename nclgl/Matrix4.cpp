...
Matrix4 Matrix4::Inverse()	{ 
	...
	m.values[0]		=	1/m.values[0];
	m.values[5]		=	1/m.values[5];
	m.values[10]	=	1/m.values[10];

	return m;

}

Vector3 Matrix4::GetForwardRotationVector()
{
	return Vector3(values[8], values[9], values[10]);
}

Vector3 Matrix4::GetUpRotationVector()
{
	return Vector3(values[4], values[5], values[6]);
}

Vector3 Matrix4::GetXVector()
{
