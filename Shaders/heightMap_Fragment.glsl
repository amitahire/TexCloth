...
void main ( void ) {
	...
	{		
		...
		gl_FragColor .rgb += ( snow .rgb * lightColour .rgb ) * 0.112;
	} 
	//Setup grass Part
	...
	{		 
		...
		colour += ( lightColour .rgb * sFactor ) * (worldPos_Modifly.y + 200.0f)/1000.0f;
		gl_FragColor = vec4 ( colour * atten * lambert , diffuse .a);
		gl_FragColor .rgb += ( mix(grass.rgb,snow.rgb,1-(270-worldPos_Modifly.y)/70)  * lightColour .rgb ) * 0.112;
	} 
	else if ((worldPos_Modifly.y >= 130.0f && worldPos_Modifly.y < 230.0f )){		
		vec3 colour = ( grass .rgb * lightColour .rgb );
		colour += ( lightColour .rgb * sFactor ) * (worldPos_Modifly.y + 200.0f)/1000.0f;
		gl_FragColor = vec4 ( colour * atten * lambert , diffuse .a);
		gl_FragColor .rgb += ( grass .rgb * lightColour .rgb ) * 0.112;
	} 
		...
		colour += ( lightColour .rgb * sFactor ) * (worldPos_Modifly.y + 200.0f)/1000.0f;
		gl_FragColor = vec4 ( colour * atten * lambert , diffuse .a);
		gl_FragColor .rgb += ( mix(sand.rgb,grass.rgb,1-(130-worldPos_Modifly.y)/50)  .rgb * lightColour .rgb ) * 0.112;
	}
	//Setup sand Part	
		...
		colour += ( lightColour .rgb * sFactor ) * (worldPos_Modifly.y + 200.0f)/1000.0f;
		gl_FragColor = vec4 ( colour * atten * lambert , diffuse .a);
		gl_FragColor .rgb += ( sand .rgb * lightColour .rgb ) * 0.112;
		...
		colour += ( lightColour .rgb * sFactor ) * (worldPos_Modifly.y + 200.0f)/1000.0f;
		gl_FragColor = vec4 ( colour * atten * lambert , diffuse .a);
		gl_FragColor .rgb += ( mix(seaBed.rgb,sand.rgb,1-(50-worldPos_Modifly.y)/15)  .rgb * lightColour .rgb ) * 0.112;
	}	
	else {
		vec3 colour = ( seaBed .rgb * lightColour .rgb );
		colour += ( lightColour .rgb * sFactor )* (worldPos_Modifly.y + 200.0f)/1000.0f;
		gl_FragColor = vec4 ( colour * atten * lambert , diffuse .a);
		gl_FragColor .rgb += ( seaBed .rgb * lightColour .rgb ) * 0.112;
	}	
	
	//gl_FragColor.rgb = normal;


	//vec3 BWcolor = vec3((gl_FragColor.r+gl_FragColor.g+gl_FragColor.b)/3.0,(gl_FragColor.r+gl_FragColor.g+gl_FragColor.b)/3.0,(gl_FragColor.r+gl_FragColor.g+gl_FragColor.b)/3.0);
	//gl_FragColor.rgb = BWcolor;
	//vec3 Pinkcolor = vec3(gl_FragColor.r*1.2,gl_FragColor.g*0.2,gl_FragColor.b*0.7);

	//vec3 filterColor = vec3(0.0,0.8,0);

	//gl_FragColor.rgb = gl_FragColor.rgb*filterColor + gl_FragColor.rgb;
	
	//gl_FragColor.rgb = vec3(1.2-gl_FragColor.r,1.2-gl_FragColor.g,1.2-gl_FragColor.b);
	
	//gl_FragColor = gl_FragColor*vec4(1,0,0,0.1);
