 ...
 uniform vec3 spotDirIn;
 uniform float degreeToSpot;
 ...
 void main ( void ) {
	...
	if(angleAtten >= cutoff) {
		...
		//gl_FragColor [0] = vec4(vec3(atten),1);
		
		//gl_FragColor [0] = vec4(vec3(normalize(pos - lightPos)),1);
	} else {
	//gl_FragColor [0] = vec4(0,0,1,1);
		//discard;
	}
}